...
object MinimalVariants {
    ...
    private def v(vertexid: Int, colorid: Int) = createDefinedExternal("v_" + vertexid + "_" + colorid)
    ...
    def k_colorable(vertices: List[Int], edges: Set[(Int, Int)], k: Int): Boolean = {
        ...
        var formula: FeatureExpr = True

        //         To enforce that each vertex is assigned a color, we
        //introduce formula '1.
        for (vertex <- vertices) {
            formula = formula and ((0 until k).foldRight(False)(v(vertex, _) or _))
        }
